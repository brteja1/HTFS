...
New format:
  ...
  - .tagfs.ttl: RDF with skos:broader (hierarchy) and htfs:hasTag (resource-tag links)
...
def migrate_rdf_to_split(tagfs_boundary):
    ...
    for tag_uri in g.subjects(RDF.type, SKOS.Concept):
        ...
        if tag_name.startswith("http://htfs.example.org/ontology#tag_"):
            ...
            # Add to new RDF graph (just the URI, no label needed in RDF)
            new_g.add((tag_uri, RDF.type, SKOS.Concept))
            new_g.add((tag_uri, HTFS.id, Literal(tag_id, datatype=XSD.integer)))

            # Copy skos:broader relationships
            for parent_uri in g.objects(tag_uri, SKOS.broader):
                new_g.add((tag_uri, SKOS.broader, parent_uri))
    ...
    for res_uri in g.subjects(RDF.type, HTFS.Resource):
        ...
        # Add to new RDF
        new_g.add((res_uri, RDF.type, HTFS.Resource))
        new_g.add((res_uri, HTFS.id, Literal(res_id, datatype=XSD.integer)))

        # Copy htfs:hasTag relationships
        for tag_uri in g.objects(res_uri, HTFS.hasTag):
            new_g.add((res_uri, HTFS.hasTag, tag_uri))
...
def rebuild_from_rdf(tagfs_boundary):
    """
    Rebuild SQLite database from existing RDF file.
    Use this if the SQLite database is corrupted but RDF is intact.
    """
    ttl_path = os.path.join(tagfs_boundary, ".tagfs.ttl")
    db_path = os.path.join(tagfs_boundary, ".tagfs.db")
